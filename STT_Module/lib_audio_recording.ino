...
// #include "FFat.h"              // also needed, but already included in Main.ino */
...
#define SAMPLE_RATE             16000  // typical values: 8000 .. 44100, use e.g 8K (and 8 bit mono) for smallest .wav files  
                                      ...
                                      //6800 is the minimum I would reccommend as Human speech range to about 3400 and double that is 6800 in case you really really need small
...
#define GAIN_BOOSTER_I2S        16     // original I2S streams is VERY silent, so we added an optional GAIN booster for INMP441
                                       ...
                                       // If you set this value too high it might not recognized your voice in low background noise
...
{ .clk_cfg  =   // instead of macro 'I2S_STD_CLK_DEFAULT_CONFIG(SAMPLE_RATE),'
  ...
  { // hint: always using _16BIT because I2S uses 16 bit slots anyhow (even in case I2S_DATA_BIT_WIDTH_8BIT used !)
    ...
    .msb_right = true,  // .. or [.msb_right = true] to avoid overdriven and distorted signals on I2S microphones
  },
  .gpio_cfg =   
...
bool Record_Start( String audio_filename ) 
{
  ...
  { 
    ...
    {  LittleFS.remove(audio_filename); DebugPrintln("\n> Existing AUDIO file removed.");
    }  else {DebugPrintln("\n> No AUDIO file found");}
    // Kalo WAV header
    File audio_file = LittleFS.open(audio_filename, FILE_WRITE);
  ...
  { 
    ...
    */ 
    File audio_file = LittleFS.open(audio_filename, FILE_APPEND);
    if (audio_file)
    {  
...
bool Record_Available( String audio_filename, float* audiolength_sec ) 
{
  ...
  { 
    ...
    File audio_file = LittleFS.open(audio_filename, "r+");
